...
impl<'a> DeviceHandle<'a> {
   ...
   pub fn exposure_abs_range(&self) -> Result<Range<u32>> {
       unsafe {
           ...
           let err = uvc_get_exposure_abs(self.devh.as_ptr(), min.as_mut_ptr(), uvc_req_code_UVC_GET_MIN).into();
           if err != Error::Success { return Err(err); }

           let err = uvc_get_exposure_abs(self.devh.as_ptr(), max.as_mut_ptr(), uvc_req_code_UVC_GET_MAX).into();
           if err != Error::Success { return Err(err); }

           let err = uvc_get_exposure_abs(self.devh.as_ptr(), step.as_mut_ptr(), uvc_req_code_UVC_GET_RES).into();
           if err != Error::Success { return Err(err); }

           let err = uvc_get_exposure_abs(self.devh.as_ptr(), def.as_mut_ptr(), uvc_req_code_UVC_GET_DEF).into();
           if err != Error::Success { return Err(err); }
   ...
   pub fn gain_range(&self) -> Result<Range<u16>> {
       unsafe {
           ...
           let err = uvc_get_gain(self.devh.as_ptr(), min.as_mut_ptr(), uvc_req_code_UVC_GET_MIN).into();
           if err != Error::Success { return Err(err); }

           let err = uvc_get_gain(self.devh.as_ptr(), max.as_mut_ptr(), uvc_req_code_UVC_GET_MAX).into();
           if err != Error::Success { return Err(err); }

           let err = uvc_get_gain(self.devh.as_ptr(), step.as_mut_ptr(), uvc_req_code_UVC_GET_RES).into();
           if err != Error::Success { return Err(err); }

           let err = uvc_get_gain(self.devh.as_ptr(), def.as_mut_ptr(), uvc_req_code_UVC_GET_DEF).into();
           if err != Error::Success { return Err(err); }
